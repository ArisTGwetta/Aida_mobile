...
// INTENT ROUTER — translates user text + context into deterministic tool intents.
(function () {
      ...
      capabilities: [
        {
          intent: "conversation",
          description:
            "Use for ordinary conversation, creative play, emotional presence, or when no tool/action is needed.",
          autoRun: false
        },
        {
          intent: "web_search",
          description:
            "Use for explicit current web lookup requests, including underspecified follow-ups like dates, prices, latest picture, or where to buy when recent turns identify the subject.",
          autoRun: true
        },
        {
          intent: "memory_search",
          description:
            "Use for finding something in Aida's indexed private/project memory, logs, or briefcases.",
          autoRun: true
        },
        {
          intent: "show_sources",
          description:
            "Use when the user asks for source traces, refs, citations, or where a previous memory answer came from.",
          autoRun: true
        },
        {
          intent: "memory_note",
          description:
            "Use when the user asks Aida to remember, note, mark, or save a statement for later sleep processing.",
          autoRun: true
        },
        {
        ...
        },
        {
          ...
          description:
            "Use for future reminders, email, calendar, or other outside-world actions. Requires confirmation and an installed tool.",
          autoRun: false
        },
        {
          intent: "clarify",
          description:
            "Use when the desired tool/action is unclear or missing required details.",
          autoRun: false
        },
        {
          intent: "none",
  ...
  function normalizeIntent(value) {
    ...
    const intent = supported.includes(value?.intent) ? value.intent : "none";
    ...
    const needsText = ["web_search", "memory_search", "show_sources"].includes(intent)
      ...
      : true;
    const canAutoRun = ["web_search", "memory_search", "show_sources", "memory_note"].includes(
      intent
    );
    return {
      intent,
      ...
      requiresTool: Boolean(
        value?.requiresTool && !["conversation", "none"].includes(intent)
      ),
      requiresConfirmation: Boolean(value?.requiresConfirmation),
      autoRun:
        canAutoRun &&
        !value?.requiresConfirmation &&
        confidence >= CONFIDENCE_AUTO_RUN &&
        needsText
    };
  }
  ...
  async function infer(userText, options = {}) {
    ...
    try {
      ...
      log(
        `INTENT: ${normalized.intent}, confidence=${normalized.confidence}.`,
        normalized.autoRun ? "log-blue" : "log-amber"
      );
      return normalized;
    } catch (error) {
      runtime().context.lastIntentRoute = {
  ...
  async function composeToolReply(payload = {}, options = {}) {
    ...
    try {
      ...
      log(
        `INTENT: tool reply composer unavailable (${error.message}).`,
        "log-amber"
      );
      return null;
    }
  ...
  if (window.AIDA_MODULES) {
    window.AIDA_MODULES.register({
      ...
      verifies: [
        "natural tool requests are translated into narrow deterministic intents before full conversation fallback"
      ]
    });
  }
})();
