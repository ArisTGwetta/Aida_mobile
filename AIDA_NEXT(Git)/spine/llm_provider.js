...
(function () {
  ...
  async function callMessages(messages, options = {}) {
    ...
    if (!ready.pass) {
      throw new Error(`LLM route is not ready: ${ready.missing.join(", ")}.`);
    }
    ...
    const maxOutputTokens =
      options.maxOutputTokens || config().maxOutputTokens || 700;
    const extraBody =
    ...
    if (!response.ok) {
      const detail =
        data?.error?.message || data?.detail || `HTTP ${response.status}`;
  ...
  async function callWebSearch(query, options = {}) {
    ...
    const endpoint = FIXED_ENDPOINTS.openai;
    ...
    const body = {
      ...
      max_output_tokens: options.maxOutputTokens || config().maxOutputTokens || 700
    };
    ...
    const text = extractOutputText(data);
    if (!text) {
      throw new Error("OpenAI web search did not contain text output.");
    }
  ...
  if (window.AIDA_MODULES) {
    window.AIDA_MODULES.register({
      ...
      verifies: [
        "OpenAI and xAI use fixed official endpoints; Ollama routes send no bearer key; explicit OpenAI web retrieval returns source metadata"
      ]
    });
  }
})();
