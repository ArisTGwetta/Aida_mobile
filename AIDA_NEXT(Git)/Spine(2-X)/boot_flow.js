...
async function continueWake() {
    // 1. Arrive first — UI must exist before messages
    arrive();

    // 2. Build messages AFTER arrival
    try {
        buildMessages();
        log("WAKE: LLM messages assembled after arrival.", "log-blue");
    } catch (err) {
        log(`WAKE: Message builder skipped during wake: ${err.message}`, "log-amber");
    }
